/*************** *************************************
* KOKO-mallin SOTE-simulointiohjelma, data 2017     *
 ...
 *Käytetäänkö hintaindeksejä vai parametrien reaaliarvoja?;
%LET OUT = 1;

%LET HINTAIND = 1; * Käytetäänkö hintaindeksejä lääkkeiden, yksityisen terveydenhuollon ja matkakorvausten osalta (0/1);
%LET TULOSLAAJ = 1;
%LET MAKSUTASO=KUNTA;*Asiakasmaksujen tasot: TYHJÄ = LAINSÄÄDÄNTÖ, KUNTA = kuntien asiakasmaksutasot v. 2017;
%LET TAKSAT=1;      * Lasketaan lääkkeiden taksat (1=kyllä, 0=ei);
...
%LET TULOSNIMI_AS = AS_TULOS;
%LET TULOSNIMI_SH = SH_TULOS;
***************;
%LET TULOSNIMI_LA = LA_TULOS;
...
%LET TYYPPI_KOKO = SIMULX;	* Parametrien hakutyyppi: SIMUL (vuosikeskiarvo) tai SIMULX (parametrit haetaan tietylle kuukaudelle);
...
%LET TULOSNIMI_KOKO = koko_simul&hintaind._&lvuosi; * Simuloidun tulostiedoston nimi;
...
%LET KOKOindikaattorit = 0; * Tulonjakoindikaattorit (1 jos ajetaan, 0 jos ei) ;
...
/* Osamallien ohjausparametreja */
  	...
  	* Muuttujien poiminta osamalleissa (1 jos ajetaan, 0 jos ei). HUOM! APUMUUTTUJIA EI SAA LUODA JOS PARAMETREJA ON MUUTETTU;
...
%LET LAKIMAK_TIED_TO = TOIMTUKIlakimakrot_asmaksu;
...
%LET LAKIMAK_TIED_SH = SAIRHKORVlakimakrot;
%LET LAKIMAK_TIED_AS = ASMAKSUlakimakrot_kunta21kun;
***************;
%LET LAKIMAK_TIED_LA = LAAKElakimakrot;
...
%LET SIMUL_TIED_TO = TOIMTUKIsimul_sotekk;
...
%LET SIMUL_TIED_SH = SAIRHKORVsimul;
%LET SIMUL_TIED_AS = ASMAKSUsimul_koko;
%LET SIMUL_TIED_LA = LAAKEsimul_JT;
...
%LET PLLISA = pllisa;
%LET PTOIMTUKI = ptoimtuki_asmaksu;
%LET PKANSEL = pkansel;
%LET PVERO = pvero;
...
%LET PPHOITO = pphoito;
%LET PSAIRHKORV = psairhkorv;
%LET PYKSTAKSA = pykstaksa;
%LET PASMAKSU = pasmaksu2021;
%LET PLAAKE = plaake;
...
%LET MUUTTUJAT = SAIRVAK_SIMUL SAIRVAK_DATA TTURVA_SIMUL TTURVA_DATA KOTIHTUKI_SIMUL KOTIHTUKI_DATA
		...
		KAYTTULO_SIMUL ktu KAYTTULO_DATA ASMAKSU_SIMUL JULKSUUNTH LAAKE_SIMUL MATKAOMAV_SIMUL YKSSUU_SIMUL YKSTERV_SIMUL TOIMTUKISOTE KTU_DT KTU_VAHSOTE_DT KTU_VAHSOTE
		; 	 
...
%IF &KOKOpoiminta = 1 %THEN %DO;
	...
	(KEEP = hnro knro asko trpl trplkor anstukor tapur tulkp tmpt tkust tepalk
	 tmeri tlue2 tpalv trespa tpturva vthm tpjta
	 tyhtat tyhthav yrtukor tpalv2 telps1 telps2 telps5 ttyoltuk tlakko tpalv2a
	 ...
	 tvakpr kokorve hlakav tpotel tmuuel tulkel teanstu tmluoko tmtatt hkuto amstipe
	 ...
	 tansel ttyoel tkansel tperhel takuuel tkopira tkuntra teleuve
	 toyjmyv toyjmav toyjmyvvap toyjmavvap
	 tyhtmy tyhtmat tyhtate tyhtmpot tyhtpote tmaat1evyr tmaat1pevyr
	...
	MUUT_EL = MAX(SUM(tansel, ttapel, tlapel, tpotel, teanstu, tmuuel, tulkel, ttyoel, teleuve), 0);
	...
	YRIT_ANSIO = SUM(tmaat1evyr, tmaat1pevyr, tpjta, tliik1evyr, tliikpevyr, tporo1evyr, tyhtmatevyr, tyhtateevyr, SUM(tyhtat, -tyhtmat, -tyhtate), tyhthav, tmtatt, yrtukor);

	/* Yritystuloja pääomatuloina (yrittäjävähennystä ei ole tehty) */

	YRIT_POTULO = SUM(tmaat2evyr, tmaat2pevyr, tliik2evyr, tliik2pevyr, tporo2evyr, tyhtmpotevyr, tyhtpoteevyr, SUM(tyhtpot, -tyhtmpot, -tyhtpote), MAX(tmetsp, 0), MAX(tmetspp, 0), MAX(tyhtmy, 0), tvaksp);
...
%IF &ASMAKSU = 1 %THEN %DO;
	%INCLUDE "&LEVY&KENO&HAKEM&KENO.SIMUL_&MVUOSI&KENO&SIMUL_TIED_AS..sas";
%END;

%IF &SAIRHKORV = 1 %THEN %DO;
	%INCLUDE "&LEVY&KENO&HAKEM&KENO.SIMUL_&MVUOSI&KENO&SIMUL_TIED_SH..sas";
%END;

/* Reseptilääkkeet */
%IF &LAAKE = 1 %THEN %DO;
	%INCLUDE "&LEVY&KENO&HAKEM&KENO.SIMUL_&MVUOSI&KENO&SIMUL_TIED_LA..sas";
%END;
...
%IF &SAIRVAK = 1 OR &TTURVA = 1 OR &KANSEL = 1 OR &KOTIHTUKI = 1 OR &OPINTUKI = 1 OR &VERO = 1 OR &KIVERO = 1
	OR &LLISA = 1 OR &ELASUMTUKI = 1 OR &ASUMTUKI = 1 OR &PHOITO = 1 &ASMAKSU OR &SAIRHKORV OR &TOIMTUKI = 1 OR &LAAKE = 1 %THEN %DO;
	...
	DATA STARTDAT.START_KOKO;
		...
		%IF &ASMAKSU = 1 %THEN %DO;
			TEMP.&TULOSNIMI_AS
			(KEEP = hnro TASAMAKSU JULKSUUNTH)
		%END;

		%IF &SAIRHKORV = 1 %THEN %DO;
			TEMP.&TULOSNIMI_SH._YK
			(KEEP = hnro YKSSUU_SIMUL YKSTERV_SIMUL )

			TEMP.&TULOSNIMI_SH._MA
			(KEEP = hnro MATKAOMAV_SIMUL)
		%END;

		/* Reseptilääkkeet */
		%IF &LAAKE = 1 %THEN %DO;
			TEMP.&TULOSNIMI_LA
			(KEEP = hnro LAAKE_SIMUL)
		%END;
		...
		%IF &TOIMTUKI = 1 %THEN %DO;
			TEMP.&TULOSNIMI_TO
			(KEEP = hnro TOIMTUKI TOIMTUKISOTE);* KOTIHTULOSM RENAME=KOTIHTULOSM=KOTIHTULOSM2)
		%END;
...
%IF &ASMAKSU = 0 %THEN %DO;
	ASMAKSU_SIMUL = 0;
	JULKSUUNTH=0;
%END;
%ELSE %DO;
	ASMAKSU_SIMUL = TASAMAKSU ; DROP TASAMAKSU;
%END;

%IF &SAIRHKORV = 0 %THEN %DO;
	YKSSUU_SIMUL=0; YKSTERV_SIMUL = 0;
	MATKAOMAV_SIMUL = 0;
%END;
%IF &LAAKE = 0 %THEN %DO;
	LAAKE_SIMUL = 0;
...
KTU_VAHSOTE = MAX(SUM(KAYTRAHATULO_SIMUL, -ASMAKSU_SIMUL, -MATKAOMAV_SIMUL, -YKSSUU_SIMUL, -YKSTERV_SIMUL, -LAAKE_SIMUL,- JULKSUUNTH,-TOIMTUKI_SIMUL, TOIMTUKISOTE), 0);
KTU_VAHSOTE_ILMTOT =SUM(KAYTRAHATULO_SIMUL, -ASMAKSU_SIMUL, -MATKAOMAV_SIMUL, -YKSSUU_SIMUL, -YKSTERV_SIMUL, -LAAKE_SIMUL,- JULKSUUNTH);

/*
KTU_VAHSOTE_DT = MAX(0, SUM(KTU_VAHSOTE,-TOIMTUKISOTE,TOIMTUKI_DATA));
KTU_DT= MAX(0, SUM(KAYTRAHATULO_SIMUL,-TOIMTUKI_SIMUL,TOIMTUKI_DATA));  
*/
...
ARRAY PISTE 
    ...
    KIVEROYHT_SIMUL KIVEROYHT_DATA TOIMTUKISOTE /* tämä on lisättyä */ LAAKE_SIMUL JULKSUUNTH YKSSUU_SIMUL YKSTERV_SIMUL;
DO OVER PISTE;
...
ASMAKSU_SIMUL='Julk.terveydenhuollon maksut, MALLI'
JULKSUUNTH='Julk.suun terveydenhuollon maksut, MALLI'
YKSTERV_SIMUL='Yksit. terveydenhuollon maksut, MALLI'
YKSTERV_SIMUL='Yksit. suun terveydenhuollon maksut, MALLI'
MATKAOMAV_SIMUL='Matkakustannusten omavastuut, MALLI'
;
RUN;
...
%IF &KOKOindikaattorit=1 %THEN %DO;
	%KoyhInd(&RAJALKM, &KRAJA1, &KRAJA2, &KRAJA3, TEMP.&TULOSNIMI_KOKO, jasenia, &PAINO, &TULO, &KULUYKS, knro, desmod, 1);
	%KoyhInd(&RAJALKM, &KRAJA1, &KRAJA2, &KRAJA3, TEMP.&TULOSNIMI_KOKO, jasenia, &PAINO, KTU_VAHSOTE, &KULUYKS, knro, desmod, 1);
	%KoyhInd(&RAJALKM, &KRAJA1, &KRAJA2, &KRAJA3, TEMP.&TULOSNIMI_KOKO, jasenia, &PAINO, KTU_VAHSOTE_DT, &KULUYKS, knro, desmod, 1);
	%KoyhInd(&RAJALKM, &KRAJA1, &KRAJA2, &KRAJA3, TEMP.&TULOSNIMI_KOKO, jasenia, &PAINO, KTU_DT, &KULUYKS, knro, desmod, 1);
%END;
...
%PUT %SYSFUNC(IFC(&LAAKE = 1 ,     LAAKE        kyllä, LAAKE        ei));
%MEND INFO;
